...
public class UserController {
    ...
    public ApiResponse<Users> createUser(
            @Parameter(description = "User details", required = true) @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "User object that needs to be added", content = @Content(schema = @Schema(implementation = Users.class))) @RequestBody Users user) {
    ...
    public ApiResponse<Users> updateUser(
            @Parameter(description = "ID of the user to update", required = true, example = "1") @PathVariable Long id,
            @Parameter(description = "Updated user details", required = true) @RequestBody Users userDetails) {
        Users user = userRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("User with ID " + id + " not found"));
    ...
    public ApiResponse<Void> deleteUser(
            @Parameter(description = "ID of the user to delete", required = true, example = "1") @PathVariable Long id) {
